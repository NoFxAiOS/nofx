...
func (s *Server) setupRoutes() {
	...
	api := s.router.Group("/api")
	{
		...
		// 市场数据配置管理（无需认证）
		api.GET("/market-configs", s.handleGetMarketConfigs)
...
type CreateTraderRequest struct {
	...
	MarketDataConfig     string  `json:"market_data_config"`     // 市场数据配置名称
...
func (s *Server) handleCreateTrader(c *gin.Context) {
	...
	if req.MarketDataConfig != "" {
		marketDataConfig = req.MarketDataConfig
	}
	// 创建交易员配置（数据库实体）
...
func (s *Server) handleUpdateTrader(c *gin.Context) {
	...
	// 设置市场数据配置默认值
	marketDataConfig := existingTrader.MarketDataConfig // 保持原值
	if req.MarketDataConfig != "" {
		marketDataConfig = req.MarketDataConfig
	}
...
// handleGetMarketConfigs 获取所有市场数据配置列表
func (s *Server) handleGetMarketConfigs(c *gin.Context) {
	configs, err := market.ListMarketConfigs()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("获取市场数据配置列表失败: %v", err)})
		return
	}
