...
func (s *Server) setupRoutes() {
	...
	api := s.router.Group("/api")
	{
		...
		protected.GET("/trade-records/export", s.handleExportTradeRecords)
		}
	}
}
...
func (s *Server) handleTradeRecords(c *gin.Context) {
	...
	// 获取所有交易记录（扩大窗口以确保捕获所有已平仓交易）
	// 使用较大的回看周期，确保能匹配到所有开仓记录
	performance, err := trader.GetDecisionLogger().AnalyzePerformance(10000)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"records": performance.RecentTrades,
	})
}

// handleExportTradeRecords 导出交易记录为CSV
func (s *Server) handleExportTradeRecords(c *gin.Context) {
	_, traderID, err := s.getTraderFromQuery(c)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	...
	// 构建CSV内容
	csvContent := "币种,方向,数量,杠杆,开仓价,平仓价,持仓市值(USDT),保证金(USDT),盈亏(USDT),盈亏百分比,持仓时长,开仓时间,平仓时间\n"
	
	for _, trade := range performance.RecentTrades {
		csvContent += fmt.Sprintf("%s,%s,%.8f,%d,%.8f,%.8f,%.2f,%.2f,%.2f,%.2f%%,%s,%s,%s\n",
			trade.Symbol,
			trade.Side,
			trade.Quantity,
			trade.Leverage,
	...
	// 设置响应头
	fileName := fmt.Sprintf("trade_records_%s_%s.csv", traderID, time.Now().Format("20060102_150405"))
...
func (s *Server) Start() error {
	...
	log.Printf("  • GET  /api/trade-records?trader_id=xxx - 指定trader的交易记录列表")
	log.Printf("  • GET  /api/trade-records/export?trader_id=xxx - 导出指定trader的交易记录为CSV")
	log.Println()
