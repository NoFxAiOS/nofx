...
func (s *Server) handleUpdateTrader(c *gin.Context) {
	...
	// 📝 日志：记录原始请求体
	body, _ := c.GetRawData()
	log.Printf("📝 [UPDATE_TRADER] 原始请求体: %s", string(body))
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		log.Printf("❌ [UPDATE_TRADER] JSON解析失败: %v", err)
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	// 📝 日志：记录接收到的系统提示词模板
	log.Printf("📝 [UPDATE_TRADER] 接收到 system_prompt_template: '%s'", req.SystemPromptTemplate)
	...
	// 设置系统提示词模板，允许更新
	systemPromptTemplate := req.SystemPromptTemplate
	log.Printf("📝 [UPDATE_TRADER] 处理 system_prompt_template - 旧值: '%s', 请求值: '%s'", existingTrader.SystemPromptTemplate, req.SystemPromptTemplate)
	if systemPromptTemplate == "" {
		systemPromptTemplate = existingTrader.SystemPromptTemplate // 保持原值
		log.Printf("📝 [UPDATE_TRADER] system_prompt_template 为空，使用旧值: '%s'", systemPromptTemplate)
	} else {
		log.Printf("📝 [UPDATE_TRADER] 使用新值 system_prompt_template: '%s'", systemPromptTemplate)
	}
	...
	trader := &config.TraderRecord{
		...
		CustomPrompt:         req.CustomPrompt,
		OverrideBasePrompt:   req.OverrideBasePrompt,
		SystemPromptTemplate: systemPromptTemplate, // ✅ 使用请求值或保持原值
		IsCrossMargin:        isCrossMargin,
		ScanIntervalMinutes:  scanIntervalMinutes,
	...
	if err != nil {
		log.Printf("❌ [UPDATE_TRADER] 数据库更新失败: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("更新交易员失败: %v", err)})
		return
	}
	log.Printf("✅ [UPDATE_TRADER] 数据库更新成功，system_prompt_template: '%s'", systemPromptTemplate)
...
func (s *Server) handleGetTraderConfig(c *gin.Context) {
	...
	if err != nil {
		log.Printf("❌ [GET_TRADER_CONFIG] 获取交易员配置失败: traderID=%s, error=%v", traderID, err)
		c.JSON(http.StatusNotFound, gin.H{"error": fmt.Sprintf("获取交易员配置失败: %v", err)})
		return
	}

	// 📝 日志：记录从数据库获取到的系统提示词模板
	log.Printf("📝 [GET_TRADER_CONFIG] 从数据库获取 system_prompt_template: '%s' (traderID: %s)", traderConfig.SystemPromptTemplate, traderID)
	...
	// 📝 日志：记录返回给前端的系统提示词模板
	log.Printf("📝 [GET_TRADER_CONFIG] 返回给前端 system_prompt_template: '%s' (traderID: %s)", traderConfig.SystemPromptTemplate, traderID)
