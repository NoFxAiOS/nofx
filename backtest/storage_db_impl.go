...
func saveCheckpointDB(runID string, ckpt *Checkpoint) error {
	...
	`)
	_, err = persistenceDB.Exec(query, runID, data)
	return err
}

func loadCheckpointDB(runID string) (*Checkpoint, error) {
	var payload []byte
	query := convertQuery(`SELECT payload FROM backtest_checkpoints WHERE run_id = ?`)
	err := persistenceDB.QueryRow(query, runID).Scan(&payload)
	if err != nil {
...
func saveConfigDB(runID string, cfg *BacktestConfig) error {
	...
	`)
	_, err = persistenceDB.Exec(insertQuery, runID, userID, data, template, cfg.CustomPrompt, cfg.OverrideBasePrompt, cfg.AICfg.Provider, cfg.AICfg.Model, now, now)
	if err != nil {
		return err
	}
	updateQuery := convertQuery(`
		...
		WHERE run_id = ?
	`)
	_, err = persistenceDB.Exec(updateQuery, userID, data, template, cfg.CustomPrompt, cfg.OverrideBasePrompt, cfg.AICfg.Provider, cfg.AICfg.Model, runID)
	return err
}
...
func loadRunIDsDB() ([]string, error) {
	rows, err := persistenceDB.Query(convertQuery(`SELECT run_id FROM backtest_runs ORDER BY updated_at DESC`))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func loadDecisionTraceDB(runID string, cycle int) (*store.DecisionRecord, error) {
	...
	var err error
	if cycle > 0 {
		query = convertQuery(`SELECT payload FROM backtest_decisions WHERE run_id = ? AND cycle = ? ORDER BY created_at DESC LIMIT 1`)
		rows, err = persistenceDB.Query(query, runID, cycle)
	} else {
		query = convertQuery(`SELECT payload FROM backtest_decisions WHERE run_id = ? ORDER BY created_at DESC LIMIT 1`)
		rows, err = persistenceDB.Query(query, runID)
	}
	if err != nil {
		return nil, err
	}
