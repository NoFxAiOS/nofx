...
func (d *Database) createTables() error {
	...
	alterQueries := []string{
		...
		`ALTER TABLE traders ADD COLUMN market_data_config TEXT DEFAULT 'default'`,     // 市场数据配置名称
...
type TraderRecord struct {
	...
	MarketDataConfig     string    `json:"market_data_config"`     // 市场数据配置名称（对应market_configs文件夹下的JSON文件名，如"default"）
...
func (d *Database) CreateTrader(trader *TraderRecord) error {
	_, err := d.db.Exec(`
		INSERT INTO traders (id, user_id, name, ai_model_id, exchange_id, initial_balance, scan_interval_minutes, is_running, btc_eth_leverage, altcoin_leverage, trading_symbols, use_coin_pool, use_oi_top, custom_prompt, override_base_prompt, system_prompt_template, market_data_config, is_cross_margin)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, trader.ID, trader.UserID, trader.Name, trader.AIModelID, trader.ExchangeID, trader.InitialBalance, trader.ScanIntervalMinutes, trader.IsRunning, trader.BTCETHLeverage, trader.AltcoinLeverage, trader.TradingSymbols, trader.UseCoinPool, trader.UseOITop, trader.CustomPrompt, trader.OverrideBasePrompt, trader.SystemPromptTemplate, trader.MarketDataConfig, trader.IsCrossMargin)
	return err
}
...
func (d *Database) GetTraders(userID string) ([]*TraderRecord, error) {
	rows, err := d.db.Query(`
		SELECT id, user_id, name, ai_model_id, exchange_id, initial_balance, scan_interval_minutes, is_running,
		       ...
		       COALESCE(market_data_config, 'default') as market_data_config,
...
func (d *Database) UpdateTrader(trader *TraderRecord) error {
	_, err := d.db.Exec(`
		UPDATE traders SET
			...
			system_prompt_template = ?, market_data_config = ?, is_cross_margin = ?, updated_at = CURRENT_TIMESTAMP
		WHERE id = ? AND user_id = ?
	`, trader.Name, trader.AIModelID, trader.ExchangeID, trader.InitialBalance,
		trader.ScanIntervalMinutes, trader.BTCETHLeverage, trader.AltcoinLeverage,
		trader.TradingSymbols, trader.CustomPrompt, trader.OverrideBasePrompt,
		trader.SystemPromptTemplate, trader.MarketDataConfig, trader.IsCrossMargin, trader.ID, trader.UserID)
	return err
}
...
func (d *Database) GetTraderConfig(userID, traderID string) (*TraderRecord, *AIModelConfig, *ExchangeConfig, error) {
	...
	err := d.db.QueryRow(`
		SELECT 
			...
			COALESCE(t.market_data_config, 'default') as market_data_config,
	...
	`, traderID, userID).Scan(
		...
		&trader.UseOITop, &trader.CustomPrompt, &trader.OverrideBasePrompt,
		&trader.SystemPromptTemplate, &trader.MarketDataConfig, &trader.IsCrossMargin,
		&trader.CreatedAt, &trader.UpdatedAt,
