...
Шаг 2: Настройка главного ключа шифрования (рекомендуется)

⚠️ Важная настройка безопасности: защитите ваши API-ключи

NOFX использует шифрование AES-256-GCM для хранения всей конфиденциальной информации (API-ключи бирж, ключи AI-моделей и т.д.). Перед первым запуском настоятельно рекомендуется установить главный ключ шифрования:
# 1. Сгенерируйте случайный главный ключ
openssl rand -base64 32
# Пример вывода: J8X5k9mPqR7tV2wN3yH4bC6dF1gL0sA8

# 2. Установите сгенерированный ключ в переменную окружения (замените на ваш реальный ключ)
...
• ✅ Главный ключ должен храниться в секрете, не добавляйте его в систему контроля версий (.env уже в .gitignore)

• ✅ Для рабочей среды настройка обязательна, иначе будет использоваться небезопасный ключ по умолчанию

• ✅ Потеря главного ключа приведет к невозможности расшифровать уже зашифрованные ключи, храните его надежно

• ⚠️ Не изменяйте ключ после его установки, иначе ранее зашифрованные данные станут нечитаемыми

#### Шаг 3: Запуск в один клик
```bash
