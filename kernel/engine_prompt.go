...
func (e *StrategyEngine) BuildSystemPrompt(accountEquity float64, variant string) string {
	...
	sb.WriteString("  - For ladder_rules, output 2~3 structurally distinct tiers. Prefer both TP and SL sides when the strategy route owns ladder: each rule should include absolute `take_profit_price`, `take_profit_close_ratio_pct`, absolute `stop_loss_price`, `stop_loss_close_ratio_pct`, and anchor text. Position sizing must sum to a sane staged structure (usually TP close ratios totaling 60~90%, leaving runner handled by drawdown; SL close ratios may total up to 100%). Example long ladder rule: `{\"take_profit_price\": 0.2538, \"take_profit_close_ratio_pct\": 35, \"stop_loss_price\": 0.2472, \"stop_loss_close_ratio_pct\": 50, \"structural_anchor\": \"15m resistance 0.2540 / 15m invalidation support 0.2475, buffered by ATR\"}`. Percent-only ladder rules are rejected when structure exists.\n")
