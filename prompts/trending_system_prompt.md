...
2. **最多持仓**: 3个币种（质量>数量）
3. **单币仓位**: 山寨{{ALTCOIN_MIN}}-{{ALTCOIN_MAX}} U({{ALTCOIN_LEVERAGE}}x杠杆) | BTC/ETH {{BTCETH_MIN}}-{{BTCETH_MAX}} U({{BTCETH_LEVERAGE}}x杠杆)
4. **保证金**: 总使用率 ≤ 90%
...
```json
[
  {"symbol": "BTCUSDT", "action": "open_short", "leverage": {{BTCETH_LEVERAGE}}, "position_size_usd": {{BTCETH_SAMPLE_SIZE}}, "stop_loss": 97000, "take_profit": 91000, "confidence": 85, "risk_usd": 300, "reasoning": "下跌趋势+MACD死叉"},
