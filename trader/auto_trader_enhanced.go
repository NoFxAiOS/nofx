...
func (eat *EnhancedAutoTrader) Shutdown() error {
	...
	// 关闭基础交易器
	if err := eat.AutoTrader.Shutdown(); err != nil {
		return fmt.Errorf("关闭基础交易器失败: %w", err)
	}
...
func (eat *EnhancedAutoTrader) ForceSaveKellyStats() error {
	return eat.kellyManagerEnhanced.SaveStatsToFile(eat.kellyManagerEnhanced.GetConfig().SaveIntervalSeconds)
}

// 覆盖executeDecisionWithRecord以使用增强版记录功能
func (eat *EnhancedAutoTrader) executeDecisionWithRecordEnhanced(decision *decision.Decision, actionRecord *logger.DecisionAction) error {
	// 先执行基础决策
	err := eat.AutoTrader.executeDecisionWithRecord(decision, actionRecord)
	...
	if decision.Action == "close_long" || decision.Action == "close_short" {
		if err == nil && actionRecord.Success {
			...
			profitPct := 0.0
			isWin := false
			if decision.ActualProfitPct != 0 {
				profitPct = decision.ActualProfitPct
				isWin = profitPct > 0
			}

			// 使用增强版记录功能
			holdingTime := time.Since(eat.getPositionOpenTime(decision.Symbol))
			eat.recordTradeResultEnhanced(decision.Symbol, isWin, profitPct, holdingTime)
		}
	}
