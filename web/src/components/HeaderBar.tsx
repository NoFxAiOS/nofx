  ...
  return (
    <nav className="fixed top-0 w-full z-50 header-bar">
            ...
            {(() => {
              ...
              return navTabs.map((tab) => (
                <button
                  ...
                  className={`text-sm font-semibold transition-all duration-300 relative focus:outline-2 focus:outline-primary whitespace-nowrap px-4 py-2.5 rounded-lg
                    ${currentPage === tab.page ? 'text-primary' : 'text-text-secondary hover:text-primary'}`}
                >
      ...
      <motion.div
        ...
        className="md:hidden overflow-hidden bg-bg-secondary border-t border-panel-border"
      >
        <div className="px-4 py-4 space-y-2">
          ...
          {(() => {
            ...
            return navTabs.map((tab) => (
              <button
                ...
                className={`block text-sm font-semibold transition-all duration-300 relative focus:outline-2 focus:outline-primary w-full text-left px-4 py-3 rounded-lg
                  ${currentPage === tab.page ? 'text-primary bg-primary-dim' : 'text-text-secondary hover:text-white hover:bg-bg-tertiary'}`}
              >
                {tab.label}
                {tab.requiresAuth && !isLoggedIn && (
                  <span className="ml-2 text-xs px-1.5 py-0.5 rounded bg-primary-dim text-primary">
          ...
          {isHomePage &&
            [
              ...
              <a
                ...
                className="block text-sm py-3 text-text-secondary hover:text-white px-4 rounded-lg hover:bg-bg-tertiary"
              >
                {item.label}
              </a>
            ))}
          ...
          {/* Social Links - Mobile */}
          <div className="pt-4 pb-2 flex items-center gap-3 border-t border-panel-border">
            <a
              href={OFFICIAL_LINKS.github}
              target="_blank"
              rel="noopener noreferrer"
              className="p-2 rounded-lg text-text-secondary bg-bg-tertiary hover:text-white hover:bg-panel-bg-hover"
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="p-2 rounded-lg text-text-secondary bg-bg-tertiary hover:text-[#1DA1F2] hover:bg-panel-bg-hover"
              onClick={() => setMobileMenuOpen(false)}
            >
              <svg
                width="18"
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="p-2 rounded-lg text-text-secondary bg-bg-tertiary hover:text-[#0088cc] hover:bg-panel-bg-hover"
              onClick={() => setMobileMenuOpen(false)}
            >
              <svg
                width="18"
          ...
          {/* Language Toggle */}
          <div className="py-3 border-t border-panel-border">
          ...
          {isLoggedIn && user && (
            <div className="mt-2 pt-4 border-t border-panel-border">
              <div className="flex items-center gap-2 px-3 py-2.5 mb-3 rounded-lg bg-bg-tertiary">
                <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold bg-primary text-black">
                  {user.email[0].toUpperCase()}
                </div>
                <div>
              ...
              {onLogout && (
                <button
                  ...
                  className="w-full px-4 py-2.5 rounded-lg text-sm font-semibold transition-colors text-center bg-danger/20 text-danger"
                >
                  {t('exitLogin', language)}
                </button>
              )}
          ...
          {!isLoggedIn &&
            currentPage !== 'login' &&
            currentPage !== 'register' && (
              <div className="space-y-3 mt-2 pt-4 border-t border-panel-border">
                <a
                  href="/login"
                  className="block w-full px-4 py-2.5 rounded-lg text-sm font-medium text-center transition-colors text-text-secondary border border-panel-border hover:text-white hover:border-white hover:bg-bg-tertiary"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  {t('signIn', language)}
                </a>
                {registrationEnabled && (
                  <a
                    href="/register"
                    className="block w-full px-4 py-2.5 rounded-lg font-semibold text-sm text-center transition-colors bg-primary text-black hover:opacity-90 hover:shadow-lg"
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    {t('signUp', language)}
                  </a>
