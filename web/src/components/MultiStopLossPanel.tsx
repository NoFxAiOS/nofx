...
interface MultiStopLossPanelProps {
  traderID: string;
  symbol: string;
  positionSide: string;
...
const MultiStopLossPanel: React.FC<MultiStopLossPanelProps> = ({
  traderID,
  symbol,
  positionSide,
  currentQuantity,
  ...
  const [stopPrices, setStopPrices] = useState<string[]>(['', '']);
  ...
  const handleAddTiers = async () => {
    ...
    if (prices.length === 0) {
      alert('Please enter at least one stop price');
      return;
    }
    ...
    try {
      ...
      alert('Stop losses set successfully');
      setStopPrices(['', '']);
    ...
    <div className="rounded-lg p-6 transition-all duration-200" style={panelStyle}>
      ...
      {showAddForm && (
        <div className="mb-4 p-4 rounded" style={{ background: 'rgba(255, 255, 255, 0.05)', border: '1px solid #2B3139' }}>
          <h4 className="text-sm font-semibold mb-3" style={{ color: '#EAECEF' }}>Add Stop Loss Tiers</h4>
          <div className="space-y-2">
            {stopPrices.map((price, idx) => (
              <div key={idx} className="flex gap-2">
                <input
                  type="number"
                  step="0.01"
                  placeholder={`Stop Price ${idx + 1}`}
                  value={price}
                  onChange={(e) => {
                    const newPrices = [...stopPrices];
                    newPrices[idx] = e.target.value;
                    setStopPrices(newPrices);
                  }}
            ...
            <button
              onClick={() => setStopPrices([...stopPrices, ''])}
...
const TierRow: React.FC<TierRowProps> = ({
  ...
  const [newPrice, setNewPrice] = useState(tier.stopPrice.toString());
    ...
    <div className="flex gap-2 p-3 rounded items-center transition-all duration-200 hover:bg-white/5" style={{ ...panelStyle, borderBottom: '1px solid #2B3139' }}>
      <div className="flex-1">
        <div className="text-sm" style={{ color: '#EAECEF' }}>
          Tier {tier.level} @ {tier.stopPrice.toFixed(2)}
        </div>
...
export default MultiStopLossPanel;
