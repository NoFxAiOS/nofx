  ...
  const t = (key: string) => {
    const translations: Record<string, Record<string, string>> = {
      ...
      enableDrawdownProtection: {
        zh: '启用回撤保护',
        en: 'Enable Drawdown Protection',
      },
      enableDrawdownProtectionDesc: {
        zh: '当利润回撤超过40%时自动平仓',
        en: 'Auto close position when profit drawdown exceeds 40%',
      },
      profitLocking: { zh: '盈利锁定', en: 'Profit Locking' },
      enableProfitLocking: { zh: '启用盈利锁定', en: 'Enable Profit Locking' },
      enableProfitLockingDesc: {
        zh: '当浮动盈亏达到设定的R倍数时，系统自动调整止损位锁定部分或全部利润。内部逻辑：每分钟检查持仓，计算当前R倍数，达到目标时执行止损调整，避免利润回吐。',
        en: 'When floating profit reaches set R multiples, the system automatically adjusts stop loss to lock partial or full profit. Internal logic: checks positions every minute, calculates current R multiple, and adjusts stop loss when targets are met to prevent profit erosion.',
      },
      profitLockTargets: { zh: '锁定目标R倍数', en: 'Lock Target R Multiples' },
      profitLockTargetsDesc: {
        zh: '输入R倍数列表（逗号分隔），例如：1,2,3。R倍数=当前利润/初始风险（入场价到止损价的距离）。系统会按顺序检查目标，达到1R时锁定一次，达到2R时再次锁定，以此类推。',
        en: 'Enter R multiples (comma-separated), e.g., 1,2,3. R multiple = current profit / initial risk (distance from entry to stop loss). The system checks targets in order, locking once at 1R, again at 2R, and so on.',
      },
      profitLockMode: { zh: '锁定模式', en: 'Lock Mode' },
      profitLockModeBreakeven: { zh: '盈亏平衡', en: 'Breakeven' },
      profitLockModeBreakevenDesc: {
        zh: '将止损移动至盈亏平衡点（考虑手续费），确保即使行情反转也不会亏损。计算公式：多单盈亏平衡价=入场价*(1+手续费率)，空单盈亏平衡价=入场价*(1-手续费率)。',
        en: 'Moves stop loss to breakeven price (considering fees), ensuring no loss even if the market reverses. Formula: long breakeven = entry price*(1+fee rate), short breakeven = entry price*(1-fee rate).',
      },
      profitLockModeTrailing: { zh: '移动止损', en: 'Trailing' },
      profitLockModeTrailingDesc: {
        zh: '根据当前R倍数动态调整止损位，锁定已获得的部分利润。例如：在3R模式下，止损会移动到确保获得2R利润的位置，即使行情反转也能保留大部分收益。',
        en: 'Dynamically adjusts stop loss based on current R multiple, locking in partial profits. For example: in 3R mode, stop loss moves to secure 2R profit, preserving most gains even if the market reverses.',
      },
      profitLockPercentage: {
        zh: '锁定仓位比例',
        en: 'Lock Position Percentage',
      },
      profitLockPercentageDesc: {
        zh: '每次达到锁定目标时平仓的比例（0.3=30%），用于实现分批锁定利润的策略。',
        en: 'Percentage of position to close when reaching lock targets (0.3=30%), used to implement partial profit locking strategy.',
      },
      feeRate: { zh: '手续费率', en: 'Fee Rate' },
      feeRateDesc: {
        zh: '用于精确计算盈亏平衡点的手续费率，默认0.05%。系统会根据此费率调整止损位，确保真正达到无亏损状态。',
        en: 'Fee rate used to accurately calculate breakeven price, default 0.05%. The system adjusts stop loss based on this rate to ensure true breakeven status.',
      },
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      ...
                      style={{ color: '#F0B90B' }}
                    >
                      {((config.profit_lock_percentage ?? 0.3) * 100).toFixed(
                        0
                      )}
                      %
                    </span>
