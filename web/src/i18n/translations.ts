...
export const translations = {
  en: {
    ...
    closeGuide: 'Close',
    binanceUserNotice: 'Binance Users Important:',
    binanceApiTypeWarning:
      'Use "Spot & Futures Trading" API, do NOT use "Unified Account API"',
    binanceApiTypeReason:
      'Reason: Unified Account API has different permission structure, which will cause order submission failure',
    binanceSetupSteps: 'Correct Setup Steps:',
    binanceSetupStep1: 'Login Binance → Account → API Management',
    binanceSetupStep2: 'Create API → Select "System Generated API Key"',
    binanceSetupStep3:
      'Check "Spot & Futures Trading" (Do NOT select Unified Account)',
    binanceSetupStep4: 'IP Restriction: Select "Unrestricted" or add server IP',
    binanceMultiAssetWarning: 'Multi-Asset Mode Users Notice:',
    binanceMultiAssetWarningDesc:
      'If you have enabled Multi-Asset Mode, you will be forced to use Cross Margin. It is recommended to disable Multi-Asset Mode to support Isolated Margin.',
    binanceOfficialGuide: 'View Binance Official Guide',
  ...
  zh: {
    ...
    closeGuide: '关闭',
    binanceUserNotice: '币安用户必读：',
    binanceApiTypeWarning: '使用「现货与合约交易」API，不要用「统一账户 API」',
    binanceApiTypeReason: '原因：统一账户 API 权限结构不同，会导致订单提交失败',
    binanceSetupSteps: '正确配置步骤：',
    binanceSetupStep1: '登录币安 → 账户 → API管理',
    binanceSetupStep2: '创建API → 选择「系统生成API密钥」',
    binanceSetupStep3: '勾选「现货与合约交易」（不要选统一账户）',
    binanceSetupStep4: 'IP限制：选择「不限制」或添加服务器IP',
    binanceMultiAssetWarning: '多资产模式用户注意：',
    binanceMultiAssetWarningDesc:
      '如果您开启了多资产模式，将被强制使用全仓保证金，建议关闭多资产模式以支持逐仓保证金。',
    binanceOfficialGuide: '查看币安官方教程',
    whitelistIP: '白名单IP',
