...
1. 等待构建完成（约3-5分钟）
2. 点击 **"Domains"** 标签
3. 记录URL，例如：`https://nofx-backend-123.railway.app`
4. 测试访问：`https://nofx-backend-123.railway.app/health`
5. 应该返回：`{"status":"ok"}`
...
```bash
# 浏览器访问
https://nofx-backend-123.railway.app/health
...
if ! curl -f http://localhost:8080/health > /dev/null 2>&1; then
    echo "后端服务异常" | mail -s "NOFX告警" admin@example.com
    ./start.sh restart
fi
